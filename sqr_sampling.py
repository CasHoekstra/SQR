...
if tau_argv is None:
    ...
    for regression_dataset in regression_dataset_namestry:
        try:
            ...
            process_fold_scores("SQR", regression_dataset, fold_scores_sqr, results70)
            process_fold_scores("LightGBM", regression_dataset, fold_scores_lgb, results70)
            process_fold_scores("DecisionTree", regression_dataset, fold_scores_tree, results70)
            process_fold_scores("LinearQuantile", regression_dataset, fold_scores_linear, results70)
        except Exception as e:
            print(f"Error processing {regression_dataset}: {e}")
    ...
    for regression_dataset in regression_dataset_namestry:
        try:
            ...
            for train_index, test_index in kf.split(X):
                ...
                sqr_sample_size = min(train_X.shape[0], sample_size)
                sqr_sample_index = np.random.choice(np.arange(train_X.shape[0]), sqr_sample_size)
                sqr_train_X, sqr_train_y = train_X[sqr_sample_index], train_y[sqr_sample_index]
                ...
                t1 = time.time()
                modelq.fit(sqr_train_X, sqr_train_y)
                t2 = time.time()
